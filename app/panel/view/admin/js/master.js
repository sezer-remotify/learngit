(function ($) {
  ...
  $.Master = function (settings) {
    ...
    $(".panelToggle").click(function (event) {
      $("#notifyPanel").fadeToggle(150);
      event.preventDefault();
    });

    $(document).on("click", "#notifyPanel .wojo.list a", function (event) {
      var dataset = $(this).data("info");
    ...
    function notifications() {
      $.ajax({
        ...
        success: function (json) {
          if ($.trim(json.html)) {
            $("#notifyPanel .wojo.list").html(json.html);
            $("#panelToggle")
              .find("span")
    ...
    setInterval(notifications, 120 * 1000); // 2 min
    ...
    /* == Tabs == */
    /*
        $('.wojo.mason').waitForImages(true).done(function() {
            $(".wojo.mason").wMason();
        });
*/
    /* == Clonner == */
    ...
    $(document).on("click", "a.action, button.action", function () {
      ...
      $.get(url, dataset.option[0], function (data) {
        if (dataset.buttons !== false && dataset.label) {
          actions +=
            "" +
            '<div class="footer">' +
