(function ($) {
  ...
  $.Time = function (settings) {
    ...
    $("#hideTimeForm").click(function () {
      $("#timeForm").transition("fadeOut", {
        ...
        complete: function () {
          $(this).hide();
          $("#showTimeForm").show();
          if (!$("#amountField").hasClass("hidden")) {
      ...
      $.get(
        ...
        function (json) {
          if (json.type === "success") {
            ...
            var row = json.data;
            $("#showTimeForm").hide();
            if (!$("#amountField").hasClass("hidden") && !row.is_billable) {
              $("#amountField").addClass("hidden");
            }
            if ($("#amountField").hasClass("hidden") && row.is_billable) {
            ...
            var uList = $("#mUserList").find(
              "[data-value='" + row.user_id + "']"
            );
            $("input[name=user_id]").val(row.user_id);
            uList.addClass("selected");
            $("[data-dropdown='#mUserList']")
              .find("span")
              .text(uList.data("html"));

            // var jList = $("#mJobList").find(
            //   "[data-value='" + row.job_id + "']"
            // );
            // $("input[name=job_id]").val(row.job_id);
            // jList.addClass("selected");
            // $("[data-dropdown='#mJobList']")
            //   .find("span")
            //   .text(jList.data("html"));

            $("#timeForm input[name=hours]").val(row.hours);
            $("#timeForm input[name=amount]").val(row.amount);
    ...
    $("#mBillable").change(function () {
      $("#amountField").toggleClass("hidden");
    });
