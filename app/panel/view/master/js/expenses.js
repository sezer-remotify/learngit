(function($) {
    ...
    $.Expense = function(settings) {
        ...
        $('#showExpForm').click(function() {
            $(this).hide();
            $('#expForm').css("display", "block").transition("fadeIn", {
                duration: 200
            });
        });

        $('#hideExpForm').click(function() {
            $('#expForm').transition("fadeOut", {
                duration: 200,
                complete: function() {
                    $(this).hide();
                    $('#showExpForm').show();
        ...
        //Edit Expense record
        $('#expHolder').on('click', '.is_edit', function() {
            var id = $(this).data('id');
            $.get(config.url + "/helper.php", {
                ...
                if (json.type === "success") {
                    $("#expForm").slideDown().addClass('loading');
                    $('html').animate({
                        scrollTop: $("#expForm").offset().top
                    }, 600);

                    var row = json.data;
                    $("#showExpForm").hide();
                    $("#expForm input[name=title]").val(row.title);
                    $("#expForm textarea[name=description]").val(row.description);
                    ...
                    $('#expForm input[name=amount]').val(row.amount).number(true, 2);
