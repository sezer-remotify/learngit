...
<div class="wojo segment">
  <div class="row gutters">
    ...
    <div class="columns auto">
      <?php if (isset($_GET['mode']) and $_GET['mode'] == "weekly") : ?>
        ...
        <a href="<?php echo Url::url("/master/projects/expenses/" . $this->row->id); ?>" class="wojo primary icon button"><i class="icon unordered list"></i></a>
      <?php else : ?>
        <a class="wojo basic disabled icon button"><i class="icon unordered list"></i></a>
        <a href="<?php echo Url::url("/master/projects/expenses/" . $this->row->id, "?mode=weekly"); ?>" class="wojo primary icon button"><i class="icon calendar"></i></a>
      <?php endif; ?>
    </div>
  </div>
  <div id="expForm" class="hide-all">
    <form method="post" id="wojo_form" name="wojo_form">
          ...
          <div class="columns screen-40 tablet-50 mobile-100 phone-100">
            <div class="wojo small block fields">
              ...
              <div class="field">
                ...
                <input type="hidden" name="created" value="<?php echo Date::doDate("dd/MM/YYYY", Date::today()); ?>">
              </div>
              <div class="field">
      ...
      <?php if (isset($_GET['mode']) and $_GET['mode'] == "weekly") : ?>
        <input type="hidden" name="return" value="<?php echo Url::url("/master/projects/expenses/" . $this->row->id, "?mode=weekly"); ?>">
      <?php else : ?>
        <input type="hidden" name="return" value="<?php echo Url::url("/master/projects/expenses", $this->row->id); ?>">
      <?php endif; ?>
    </form>
  </div>
  <div class="row">
...
<div id="expHolder">
  <?php if (isset($_GET['mode']) and $_GET['mode'] == "weekly") : ?>
    <?php include_once(MASTERBASE . "/_expenses_weekly.tpl.php"); ?>
  <?php else : ?>
    <?php include_once(MASTERBASE . "/_expenses_list.tpl.php"); ?>
  <?php endif; ?>
</div>
<script src="<?php echo MASTERVIEW; ?>/js/expenses.js"></script>
