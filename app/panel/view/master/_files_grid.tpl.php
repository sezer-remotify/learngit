              ...
              <div class="wojo small list">
                <div class="item align center">
                  <?php switch ($row->parent):
                    case "discussion":
                      echo Lang::$word->IN . ' ' . Lang::$word->MSG_DISCN . ':&nbsp;<a href="' . Url::url("/master/projects/discussions", $this->row->id) . '">' . $row->commentname . '</a>';
                      break;
                    case "task":
                      echo Lang::$word->IN . ' ' .  Lang::$word->TSK_TASK . ':&nbsp;<a href="' . Url::url("/master/projects/tasks", $this->row->id) . '">' . $row->taskname . '</a>';
                      break;
                    case "note":
                      echo Lang::$word->IN . ' ' .  Lang::$word->NOT_NOTE . ':&nbsp;<a href="' . Url::url("/master/projects/notes", $this->row->id) . '">' . $row->notename . '</a>';
                      break;
                    default:
                      //echo Lang::$word->IN . ' ' .  Lang::$word->PRJ_PROJECT . ':&nbsp;<a href="' . Url::url("/master/projects/view", $this->row->id) . '">' . $this->row->name . "</a>";
                      break;
                  ?>
                  <?php endswitch; ?>
                </div>
              </div>
              <div class="wojo small horizontal list align center">
                <div class="item">
                  <?php echo Lang::$word->BY; ?>:&nbsp; <a href="<?php echo Url::url("/master/profile/view", $row->username); ?>"><?php echo $row->created_by_name; ?></a>
                </div>
                <div class="item">
                  <?php echo Lang::$word->AT . ': ' . Date::doTime($row->created); ?>
                </div>
              </div>
            </div>
          </div>
