...
class Upload
{
	 ...
	 * @return
	 */
	public function check($uploadName, $index)
	{
		if (isset($_FILES[$uploadName])) {
			if (is_array($_FILES[$uploadName]['name']) || is_array($_FILES[$uploadName]['size'])) {
				$this->fileInfo['ext'] = substr(strrchr($_FILES[$uploadName]["name"][$index], '.'), 1);
				$this->fileInfo['name'] = basename($_FILES[$uploadName]["name"][$index]);
				$this->fileInfo['xame'] = substr($_FILES[$uploadName]["name"][$index], 0, strrpos($_FILES[$uploadName]["name"][$index], "."));
				$this->fileInfo['size'] = $_FILES[$uploadName]["size"][$index];
				$this->fileInfo['temp'] = $_FILES[$uploadName]["tmp_name"][$index];
				$this->fileInfo['type'] = $_FILES[$uploadName]["type"][$index];
				$type = explode("/", $_FILES[$uploadName]["type"][$index]);
			} else {
	 ...
	 * @return
	 */
	public function process($name, $dir, $prefix = 'SOURCE_', $fname = false, $replace = true, $index = 0)
	{
		if (!is_dir($dir)) {
			Message::$msgs['dir'] = Lang::$word->FU_ERROR12; //Directory doesn't exist!
		}
		if ($this->check($name, $index)) {
			if ($fname == false) {
