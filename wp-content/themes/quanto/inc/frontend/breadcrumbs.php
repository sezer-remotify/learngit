...
if ( ! function_exists( 'quanto_breadcrumbs' ) ) {
    function quanto_breadcrumbs ( 
        ...
        $theme = wp_get_theme();
        $text_domain =  $theme->get( 'quanto' );
        // Open list
        $breadcrumb = '<div class="page-breadcrumb"><nav aria-label="breadcrumb"><' . $list_style . ' id="' . $list_id . '" class="' . $list_class . '">';
        // Front page
        if ( is_front_page() ) {
            $breadcrumb .= '<li class="breadcrumb-item ' . $active_class . '">' . __('Home', $text_domain) . '</li>'; //get_bloginfo( 'name' )
        } else {
            $breadcrumb .= '<li class="breadcrumb-item"><a href="' . home_url() . '">' . __('Home', $text_domain) . '</a></li>'; //get_bloginfo( 'name' )
        }
        // Blog archive
        if ( 'page' == get_option( 'show_on_front' ) && get_option( 'page_for_posts' ) ) {
            $blog_page_id = get_option( 'page_for_posts' );
            if ( is_home() ) {
                $breadcrumb .= '<li class="breadcrumb-item  ' . $active_class . '">' . get_the_title( $blog_page_id )  . '</li>';
        ...
        if ( is_archive() && ! is_tax() && ! is_post_type_archive() ) {
            $breadcrumb .= '<li class="breadcrumb-item  ' . $active_class . '">';
            // Title of archive
            if ( is_category() ) {
        ...
        if ( is_singular( 'post' ) ) {
            ...
            // Post title
            $breadcrumb .= '<li class="breadcrumb-item  ' . $active_class . '">' . get_the_title() . '</li>';
        }
        // Pages
        ...
        // Search
        if ( is_search() ) {
            $breadcrumb .= '<li class="breadcrumb-item  ' . $active_class . '">' . __( 'Search', $text_domain ) . '</li>';
        }
        // 404
        if ( is_404() ) {
            $breadcrumb .= '<li class="breadcrumb-item  ' . $active_class . '">' . __( '404', $text_domain ) . '</li>';
        }
        // Custom Post Type Archive
        if ( is_post_type_archive() ) {
            $breadcrumb .= '<li class="breadcrumb-item  ' . $active_class . '">' . post_type_archive_title( '', false ) . '</li>';
        }
        // Custom Taxonomies
        if ( is_tax() ) {
            ...
            foreach ( $attached_to as $post_type ) {
                ...
                if ( ! $output && get_post_type_archive_link( $cpt_obj->name ) ) {
                    $breadcrumb .= '<li class="breadcrumb-item"><a href="' . get_post_type_archive_link( $cpt_obj->name ) . '">' . $cpt_obj->labels->singular_name . '</a></li>';
                    $output = true;
                }
            }
            // Term title
            $breadcrumb .= '<li class="breadcrumb-item  ' . $active_class . '">' . single_term_title( '', false ) . '</li>';
        }
